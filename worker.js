...
function makeAlias() {
    return (ADJECTIVES[Math.floor(Math.random() * ADJECTIVES.length)] + " " + ADJECTIVES[Math.floor(Math.random() * ADJECTIVES.length)] + " " + NOUNS[Math.floor(Math.random() * NOUNS.length)]).toLowerCase()
}
...
async function triggerEvent(scheduledTime) {
    ...
    await IDLEPAGE.put("leaderboard", JSON.stringify(filtered.slice(0, 19)));
